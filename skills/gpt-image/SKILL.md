...
compatibility: "Requires Python 3.11+ and either `gpt-image`, `uv`, or `uvx`. CLI/API calls read `OPENAI_API_KEY`, optionally read `openai_base_url` / `OPENAI_BASE_URL` for OpenAI-compatible endpoints, and may incur OpenAI API charges."
...
- Optional `openai_base_url` / `OPENAI_BASE_URL` points the SDK at an OpenAI-compatible endpoint; if unset, the SDK default base URL is used.
